    ...
    <div className="modal-overlay">
      <div className="modal-content" style={{ maxWidth: '720px' }}>
        <div className="modal-header">
          <h2>Farmer ID Card</h2>
          <button className="modal-close" onClick={() => { setShowIdCardModal(false); setIdCard(null); setIdCardImageUrl(null); }}>×</button>
        </div>
        <div className="modal-body" style={{ padding: 16 }}>
          {loadingIdCard && <div>Loading...</div>}
          {!loadingIdCard && idCard && (
            <div>
              <div style={{ textAlign: 'center', marginBottom: 12 }}>
                {idCardImageUrl ? (
                  <img src={idCardImageUrl} alt="ID Card" style={{ maxWidth: '100%', borderRadius: 8, border: '1px solid #e5e7eb' }} />
                ) : (
                  <iframe src={idCard.pngUrl} title="IDCard" style={{ width: '100%', height: 420, border: '1px solid #e5e7eb', borderRadius: 8 }} />
                )}
              </div>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                <a className="action-btn primary" href={idCard.pdfUrl} target="_blank" rel="noreferrer">Download PDF</a>
                <a className="action-btn secondary" href={idCard.pngUrl} target="_blank" rel="noreferrer">Open PNG</a>
              </div>
            </div>
          )}
        </div>
